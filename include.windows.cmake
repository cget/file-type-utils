...
function(SETUP_TOOLS)
	set(cmd_paths "")
    # TODO: These paths are not valid for VS2017 or newer
...
FUNCTION(CGET_GET_MSVC_RUNTIME name var)
    ...
    foreach(RUNTIME "142" "141" "140" "120" "110" "100" "90")
...
function(CGET_FILTER_BY_RUNTIME OUTPUT libraries RUNTIME_MATCH)
    ...
    foreach(DLL ${LOCAL_LIST})   
        ...
        if(NOT "${RUNTIME}" MATCHES "${RUNTIME_MATCH}" AND NOT "${RUNTIME}" STREQUAL "")
            # Starting with VS2015(v140) binaries are compatible with VS2017/2019(v141/v142)
            if("${RUNTIME}" MATCHES "v14" AND "${RUNTIME_MATCH}" MATCHES "v14")
                string(SUBSTRING ${RUNTIME} 4 -1 RUNTIME_TYPE)
                string(SUBSTRING ${RUNTIME_MATCH} 4 -1 RUNTIME_MATCH_TYPE)
                # Check for compatibility (release/debug match and target runtime newer than library runtime)
                if("${RUNTIME_TYPE}" MATCHES "${RUNTIME_MATCH_TYPE}" AND "${RUNTIME}" STRLESS "${RUNTIME_MATCH}")
                    MESSAGE("Allowing library runtime ${RUNTIME} with target runtime ${RUNTIME_MATCH}")
                    continue()
                endif()
            endif()
